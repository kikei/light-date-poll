...
## 画面
作成画面は開始日、終了日、メッセージ、候補上限日数、平日のみの有無を入力してフォームを作成する。作成ボタンを押すとフォーム ID と編集用 URL と投票用 URL を返す。編集画面は投票用 URL と回答状況 (UU、回答者数、内訳) を確認できる。UU は投票画面には表示しない。投票画面は候補日と現在の票数を表示する。参加者は任意の日付ボタンを押して投票する。氏名やニックネームは収集せず、回答は人数のみを表示する。投票済みの日付ボタンを再度クリックすると投票を取り消すことができる。
