...
function rid(n=8){ return rand("abcdefghijklmnopqrstuvwxyz0123456789", n); }
function rsecret(n=12){ return rand("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_", n); }
function rand(chars, n){ let s=""; for(let i=0;i<n;i++) s+=chars[Math.floor(Math.random()*chars.length)]; return s; }
...
function pickDates(startISO, endISO, limit=10, weekdaysOnly=true){
  const out=[], s=new Date(startISO), e=new Date(endISO);
  s.setHours(0,0,0,0); e.setHours(0,0,0,0);
  for(let d=new Date(s); d<=e && out.length<limit; d.setDate(d.getDate()+1)){
...
app.post("/api/forms", async (req, res) => {
  try {
    const { startDate, endDate, message, days=10, weekdaysOnly=true } = req.body || {};
    if (!startDate || !endDate || !message) return res.status(400).json({error:"missing fields"});

    const formId = rid();
    const secret = rsecret();
    const options = pickDates(startDate, endDate, Number(days)||10, !!weekdaysOnly);
...
app.get("/api/forms/:id", async (req, res) => {
  const { id } = req.params;
  const f = await pool.query("SELECT message, options FROM forms WHERE form_id=$1", [id]);
  if (!f.rowCount) return res.status(404).json({error:"not_found"});
  const rows = await pool.query("SELECT date, count FROM counts WHERE form_id=$1 ORDER BY date", [id]);
  const counts = Object.fromEntries(rows.rows.map(r => [r.date.toISOString().slice(0,10), Number(r.count)]));
  res.json({ formId:id, message: f.rows[0].message, options: f.rows[0].options, counts });
});
